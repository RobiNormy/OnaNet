...
class _DashboardState extends State<Dashboard> {
  ...
  Future<void> _activateTestUpgrade() async {
    ...
    try {
      await _startPlanPayment('pro');
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          behavior: SnackBarBehavior.floating,
          content: Text(
            'Paystack checkout opened. Your Pro plan activates after payment.',
          ),
        ),
      );
    } catch (error) {
  ...
  Future<void> _activateTestGrowth() async {
    ...
    try {
      await _startPlanPayment('growth');
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          behavior: SnackBarBehavior.floating,
          content: Text(
            'Paystack checkout opened. Your Growth plan activates after payment.',
          ),
        ),
      );
    } catch (error) {
  ...
  Future<void> _startPlanPayment(String tier) async {
    ...
    final payment = await SubscriptionService().initializePayment(
      providerId: providerId,
      tier: tier,
      email: email,
    );
    final checkout = Uri.tryParse(
      payment['authorization_url']?.toString() ?? '',
    );
    if (checkout == null ||
        !await launchUrl(checkout, mode: LaunchMode.externalApplication)) {
      throw const SubscriptionException('Could not open Paystack checkout.');
    }
  }
  ...
  Future<void> _refreshWholeDashboard() async {
    if (!mounted) return;
    final providerRefresh = AuthService().getProviderDashboardData();
    setState(() {
      _providerFuture = providerRefresh;
    });
    await Future.wait([
      providerRefresh.then<void>((_) {}),
      _refreshInbox(),
      _loadAccountAccess(),
    ]);
  }
                        ...
                        return SizedBox.expand(
                          child: SafeArea(
                            child: RefreshIndicator(
                              onRefresh: _refreshWholeDashboard,
        ...
        children: [
            ...
            child: FilledButton.icon(
              ...
              label: Text(
                loading
                    ...
                    : _selectedTier == 'free'
                    ? 'Free plan'
                    : 'Pay for ${_titleCase(_selectedTier)}',
                softWrap: true,
          ...
          Text(
            _selectedTier == 'free'
                ...
                : 'Secure payment is handled by Paystack. Plan access begins only after payment is verified.',
            textAlign: TextAlign.center,
            softWrap: true,
            style: TextStyle(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Center(child: _CurrentPlanBadge(tier: subscriptionTier)),
            const SizedBox(height: 16),
            Flexible(
              child: SingleChildScrollView(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 7),
                _CurrentPlanBadge(tier: subscriptionTier),
              ],
            ),
          ),
        ],
...
class _CurrentPlanBadge extends StatelessWidget {
  const _CurrentPlanBadge({required this.tier});

  final String tier;

  @override
  Widget build(BuildContext context) {
    final normalizedTier = switch (tier.trim().toLowerCase()) {
      'growth' => 'Growth',
      'pro' => 'Pro',
      _ => 'Free',
    };
    final color = switch (normalizedTier) {
      'Growth' => AppTheme.amberDark,
      'Pro' => AppTheme.navy,
