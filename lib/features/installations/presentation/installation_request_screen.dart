// Customer installation request form.
import 'dart:async';
...
class _InstallationRequestScreenState extends State<InstallationRequestScreen> {
  ...
  bool _loadingLandmark = false;
  String? _autoFilledLandmark;
  int _landmarkLookupId = 0;
  ...
  Future<void> _fetchGpsLocation() async {
    ...
    if (location == null && mounted) {
      ...
      await _fillNearbyLandmark(location.latitude, location.longitude);
    }
  }

  Future<void> _fillNearbyLandmark(double latitude, double longitude) async {
    final current = _landmarkController.text.trim();
    if (current.isNotEmpty && current != _autoFilledLandmark) return;

    final lookupId = ++_landmarkLookupId;
    setState(() => _loadingLandmark = true);
    final landmark = await Location.findNearbyLandmark(
      latitude,
      longitude,
    ).timeout(const Duration(seconds: 8), onTimeout: () => null);
    if (!mounted || lookupId != _landmarkLookupId) return;

    setState(() {
      _loadingLandmark = false;
      if (landmark != null &&
          (_landmarkController.text.trim().isEmpty ||
              _landmarkController.text.trim() == _autoFilledLandmark)) {
        _landmarkController.text = landmark;
        _autoFilledLandmark = landmark;
      }
    });
  }
  ...
  void _selectLocation(LocationSuggestion suggestion) {
    ...
    _fillNearbyLandmark(suggestion.latitude, suggestion.longitude);
  }
            ...
            _SectionCard(
              children: [
                ...
                TextField(
                  ...
                  onChanged: (value) {
                    if (value.trim() != _autoFilledLandmark) {
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText: _loadingLandmark
                        ? 'Finding a nearby landmark...'
                        : 'Filled automatically from your selected location; you can edit it.',
                    suffixIcon: _loadingLandmark
                        ? const Padding(
                            padding: EdgeInsets.all(14),
                            child: SizedBox.square(
                              dimension: 18,
