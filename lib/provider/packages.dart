    ...
    return ProviderFlowShell(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          DropdownButtonFormField<String>(
            initialValue: routerIncluded.first,
            decoration: InputDecoration(
            ...
            onPressed: () {
              final completedDraft = widget.draft.copyWith(
                package: ProviderPackageDraft(
                  ...
                  speedMbps: int.tryParse(_speedController.text.trim()),
                  monthlyPrice: int.tryParse(_priceController.text.trim()),
                  installationFee: int.tryParse(
                    _installationController.text.trim(),
                  ),
                  fairUsagePolicy: _fairusage.text.trim(),
                  installationPeriod: _installationperiod.text.trim(),
                ),
              );
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    '${completedDraft.package?.name ?? 'Provider registration'} draft saved.',
                  ),
                ),
              );
